---
id: c27e2935-810d-4c5f-84b1-23393d7f12ac
blueprint: homepage
title: "Australia's Leading Vocal Powerhouse"
template: home
education_section_title: Education
education_section_content:
  -
    type: paragraph
    content:
      -
        type: text
        marks:
          -
            type: bold
        text: 'As advocates for education in music – a universal language, Gospo aims to put performers on the map in a competitive industry.'
      -
        type: text
        text: ' We believe in the importance of developing the minds of the young and the young at heart. Learn from leading professionals in the industry with our structured programs, tailored workshops, seminars, coaching and individual lessons.'
  -
    type: paragraph
    content:
      -
        type: text
        marks:
          -
            type: bold
        text: 'Details for how to join us on stage for Fringe 2026 are coming very soon – please keep checking back!'
education_section_link: 'https://education.gospo.com'
events_section_title: 'Music & Events'
events_section_content:
  -
    type: paragraph
    content:
      -
        type: text
        marks:
          -
            type: bold
        text: 'Gospo provides innovative and crowd-pleasing entertainment, whatever the occasion. '
      -
        type: text
        text: 'Regardless of whether your event is intimate or grandiose, trusting us to as your agent and consultant will ensure you find the right performance, ensemble, DJ and even production to cater for the specifics needs, tastes and budget for you, your clients and guests.'
events_section_link: 'https://education.gospo.com'
updated_by: b2879438-b457-4dae-bd9e-2143be2699a2
updated_at: 1766492365
education_section_link_text: 'Enquire Now'
events_section_link_text: 'Get In Touch'
artists:
  - pages
artist_list:
  -
    id: mjgrg122
    name: 'Gospo Collective'
    description: 'Australia’s Leading Vocal Powerhouse'
    image:
      - gospo-collective.png
    type: artist
    enabled: true
  -
    id: mjgrn3nj
    name: 'Charmaine Janitha Jones'
    description: 'Performer, Educator, Arranger & Conductor'
    image:
      - charmaine-janitha-jones.png
    link: 'https://www.charmainejones.com.au/'
    type: artist
    enabled: true
artists_title: 'Providing Entertainment at the Highest Calibre'
artists_content:
  -
    type: paragraph
    content:
      -
        type: text
        text: 'From the administrative to the creative, you can trust Gospo, whose founders practice what they preach – for your next corporate function, event, community engagement or education experience.'
artist_list_title: 'Gospo Represents the Following Acts'
education_section_image: gospo-juniors.jpg
events_section_image: gospo-night-at-the-barracks.jpg
display: true
overline: 'Adelaide Fringe 2026'
heading: 'Jackson Vs. Jackson'
subheading: 'Presented by Gospo and Gluttony'
feature_display: true
feature_background_colour: '#660033'
feature_background_image: fringe-26-bg-1-1.png
feature_background_size: contain
feature_image: gospo-jvj-fb-hero-1080x1080.png
feature_background_position_x: center
feature_background_position_y: bottom
feature_background_attachment: fixed
feature_content:
  -
    type: heading
    attrs:
      level: 3
    content:
      -
        type: text
        text: 'Adelaide Fringe 2026'
  -
    type: heading
    attrs:
      level: 2
    content:
      -
        type: text
        text: 'Jackson Vs. Jackson'
  -
    type: paragraph
    content:
      -
        type: text
        marks:
          -
            type: bold
        text: 'Presented by Gospo and Gluttony'
  -
    type: paragraph
    content:
      -
        type: text
        text: 'Temperatures are rising now, with Jackson Vs Jackson, an electrifying homage to the music and legacy of Michael and Janet Jackson, led by vocal powerhouse Charmaine Janitha Jones and Adelaide’s Gospo Collective.'
  -
    type: paragraph
    content:
      -
        type: text
        text: 'This high-energy production explodes with gospel harmonies, a live band, dynamic choir, dazzling vocals and choreography that celebrates the sound, soul and style of pop’s most iconic duo. After sell-out seasons in 2018 and 2019, Gospo are back at Gluttony in 2026 for a triumphant return season.'
  -
    type: paragraph
    content:
      -
        type: text
        text: '★★★★½ “With soulful originality and uplifting energy … everyone was encouraged to get up and have a boogie!” — Play & Go Adelaide.'
  -
    type: paragraph
    content:
      -
        type: text
        text: '★★★★ “A thrilling, feel-good concert packed with extraordinary vocal talent.” — Arts Review.'
content:
  -
    type: paragraph
    content:
      -
        type: text
        marks:
          -
            type: bold
        text: 'Gospo is a reputed Adelaide-based agency specialising in an across the board service in music, in Australia and beyond. From education and technical support to a variety of shows and performances, Gospo delivers a product that fills the senses.'
  -
    type: paragraph
    content:
      -
        type: text
        text: 'From the administrative to the creative, you can trust Gospo, whose founders practice what they preach – for your next corporate function, event, community engagement or education experience.'
---
